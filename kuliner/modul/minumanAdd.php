...
$keterangan = mysqli_real_escape_string($conn, str_replace(array("\r\n", "\r", "\n"), " ", $_POST['keterangan']));
    ...
    if ($gambar_minuman) {
        ...
        if ($gambar_size <= $max_size && in_array($gambar_type, $allowed_types)) {
            ...
            // Siapkan query untuk menyimpan data minuman dan gambar dalam database
            $query = "INSERT INTO tbl_minuman (nama_minuman, daerah_minuman, keterangan, gambar_minuman) VALUES (?, ?, ?, ?)";
            if ($stmt = mysqli_prepare($conn, $query)) {
                // Mengikat parameter gambar sebagai data biner (tipe 'b' untuk BLOB)
                mysqli_stmt_bind_param($stmt, "ssss", $nama_minuman, $daerah_minuman, $keterangan, $gambar_data);
        ...
        $query = "INSERT INTO tbl_minuman (nama_minuman, daerah_minuman, keterangan) VALUES (?, ?, ?)";
        if ($stmt = mysqli_prepare($conn, $query)) {
            mysqli_stmt_bind_param($stmt, "sss", $nama_minuman, $daerah_minuman, $keterangan);
